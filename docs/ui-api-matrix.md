...
| → web UI (Micro Phase 105) | — | consumes list/create/post/cancel/regenerate-pdf | same Nest codes (`collection.*`; prompt aliases `customer_receipt.*` are not in catalog) | page `/sales/collections` | `apps/web/src/customer-receipts/*`, `apps/web/src/pages/CollectionsPage.tsx` | Nav: Sales > Collections |
