...
/** Apply equal-split amounts onto director rows (pure). */
export function applyEqualDirectorCommitments(
  directors: CapitalDirectorRow[],
  approvedBudget: number,
  investorCommitmentTotal: number,
): CapitalDirectorRow[] {
  const amounts = equalDirectorCommitments(
    approvedBudget,
    directors.length,
    investorCommitmentTotal,
  );
  return directors.map((row, index) => ({
    ...row,
    commitmentAmount: String(amounts[index] ?? 0),
  }));
}
...
export async function loadCapitalPlanDefaults(projectId: string): Promise<{
  ...
  const [active, history] = await Promise.all([
    ...
    fetchParticipantHistory(projectId, { page: 1, limit: 100 }).catch(
      () => null,
    ),
  ]);
