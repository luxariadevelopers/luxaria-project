...
import { useUpdateVendorPayment } from './useVendorPayments';
...
/**
 * Payment proof — Nest stores `paymentProof` as document id/path (no multipart route).
 */
export function PaymentProofPanel({
  ...
  const editable =
    canUpload && payment.status === VendorPaymentStatus.Draft;

  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <DialogTitle>Payment proof — {payment.paymentNumber}</DialogTitle>
      <DialogContent>
        <Stack spacing={2} data-testid="vendor-payment-proof-panel">
