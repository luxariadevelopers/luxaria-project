...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <Stack spacing={0.25}>
      ...
      <Typography variant="body2" data-testid={`vendor-bank-field-${label}`}>
        {value}
      </Typography>
    </Stack>
  );
...
/**
 * Masked bank card — account number hidden by default even when Nest decrypts.
 */
export function VendorBankCard({ vendor, canView }: Props) {
  ...
  if (!canView) {
    return (
      <PermissionDenied
        ...
        message="You need vendor.view to open vendor bank details."
        showHomeLink={false}
      />
    );
  }
  ...
  if (!bank || !hasAny) {
    return (
      <EmptyState
        ...
        description="Bank details appear after they are saved on the vendor record."
      />
    );
  }
  ...
  return (
    <Stack spacing={2} data-testid="vendor-bank-card">
        ...
        >
          <Stack spacing={0.25}>
            ...
            <Typography
              ...
              data-testid="vendor-account-display"
          ...
          {account.canReveal ? (
            <Button
              ...
              data-testid="vendor-account-reveal"
            >
