 ...
 * - `measurement.certify` — verify, reject (engineer)
 *
 * Phase brief aliases (`work_measurement.view/create/verify`) are not in the
 * Nest catalog; verify UI maps to `measurement.certify`.
 */
...
export function resolveWorkMeasurementCapabilities(
  ...
  return {
    ...
    canUpdate: canCreate,
    canSubmit: canCreate,
    canVerify: hasPermission('measurement.certify'),
    canReject: hasPermission('measurement.certify'),
    canCancel: canCreate,
