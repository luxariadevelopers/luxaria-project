...
import {
  QUOTATION_COMPARISON_APPROVAL_ENTITY,
  QUOTATION_COMPARISON_APPROVAL_MODULE,
} from './quotation-comparisons.service';

@Injectable()
export class QuotationComparisonsSeedService implements OnModuleInit {
  private readonly logger = new Logger(QuotationComparisonsSeedService.name);
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.warn(
        `Quotation comparison approval workflow seed skipped: ${
  ...
  async ensureApprovalWorkflow(): Promise<void> {
    ...
    if (!purchase || !finance) {
      this.logger.warn(
        'PURCHASE_/FINANCE_* roles missing — quotation comparison approval workflow not seeded',
      );
      return;
    }

    const systemActor = String(purchase._id);
    ...
    await this.approvalsService.upsertWorkflow(
      {
        ...
        name: 'Vendor quotation comparison recommendation',
