...
import type { ConstructionReportPayload } from './construction-reports.types';
import type { ConstructionReportsQueryDto } from './dto/construction-reports-query.dto';

@Injectable()
export class ConstructionReportsExportService {
  constructor(
  ...
  async export(
    ...
    if (format === 'pdf') {
      return {
        ...
        buffer: await this.toPdf(payload),
      };
    }
    ...
    return {
      ...
      buffer: await this.toExcel(payload),
    };
  }

  private async toExcel(payload: ConstructionReportPayload): Promise<Buffer> {
  ...
  private async toPdf(payload: ConstructionReportPayload): Promise<Buffer> {
